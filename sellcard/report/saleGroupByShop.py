...
from sellcard.models import OrderUpCard,Orders,OrderPaymentInfo
...
def index(request):
    ...
    totalDict = {'discTotal': 0, 'discCashTotal': 0, 'discCardTotal': 0, 'inSubTotal': 0, 'total_1': 0, 'total_2': 0,
                 'total_3': 0,
    ...
    for i in range(0, len(shops)):
        if shops[i]['shop_code'] != 'ZBTG':
            item = {'shop_code': '', 'disc': 0, 'disc_cash': 0, 'disc_card': 0, 'inSub': 0, 'pay_1': 0, 'pay_2': 0,
                    'pay_3': 0,
...
def remittance(request):
    ...
    endTime = str(datetime.datetime.strptime(end, '%Y-%m-%d').date() + datetime.timedelta(1))
    List = Orders.objects \
        .values('order_sn','action_type','total_amount','paid_amount','disc_amount','y_cash') \
        .filter(add_time__gte=start, add_time__lte=endTime,shop_code=shop_code) \
        .order_by('order_sn')

    #Order_Sum = OrderPaymentInfo.objects.values('order_id','pay_id').annotate(sum_value=Sum('pay_value')).aggregate(Sum('pay_value'))
    ...
    try:
        ...
        print(e)
    return render(request, 'report/saleGroupByShop/remittance.html', locals())
