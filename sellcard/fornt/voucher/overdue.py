...
def index(request):
    ...
    if request.method == 'POST':
        ...
        if mth.getReqVal(request, 'btn_value', '') == 'query':
            data_info = KfJobsCouponSn.objects.values('state','coupon_code','serial_id','used_flag').filter(voucher=voucherSn)
        ...
        if  mth.getReqVal(request, 'btn_value', '') == 'save':
            ...
            clearFlag = mth.getReqVal(request, 'clearFlag', '')
            shop = request.session.get("s_shopcode")
            name = request.session.get("s_unameChinese")
            if (shop is None or shop == ''):
                shop = '9999'
            if type == '通用券':
                UpdateCoupon(voucherSn, shop, clearFlag, spend, name)# 调用ERP过程
            ...
            msg = 5
    return render(request, 'voucher/overdue/index.html', locals())
...
def UpdateCoupon(voucherSn, shop, type, spend, name):
    """
    调存储过程核销
    ...
    cursor = conn.cursor()
    sql = "exec kgInsertCouponAcc99_past @CouponNO='" + voucherSn + "', @ShopID='" + shop \
          + "', @sValue=" + spend + ", @clearnote ='" + name + "'"+ ", @SheetType ='" + type + "'"
    cursor.execute(sql)
    conn.commit()
    cursor.close()
    conn.close()
