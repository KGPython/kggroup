    ...
    try:
        with transaction.atomic():
            ...
            ActionLog.objects.create(url=path, u_name=request.session.get('s_uname'),
                                     cards_out=cardStr + ',' + YcardStr, add_time=datetime.datetime.now())
    except Exception as e:
        res["msg_err"] = e.value
        res["msg"] = 0
        ActionLog.objects.create(url=path, u_name=request.session.get('s_uname'), cards_out=cardStr + ',' + YcardStr,
                                 add_time=datetime.datetime.now(), err_msg=e.value)
...
def info(request):
    ...
    order = Orders.objects \
        .values('shop_code', 'operator_id', 'paid_amount', 'disc_amount', 'diff_price', 'y_cash', 'buyer_name',
                'add_time') \
        .filter(order_sn=orderSn)
    infoList = OrderInfo.objects.values('card_balance', 'card_attr').filter(order_id=orderSn).annotate(
            subVal=Sum('card_balance'), subNum=Count('card_id'))
